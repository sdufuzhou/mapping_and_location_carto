...
class MappingAndLocationImpl {
 public:
  ...
  using CallBackEvent = gomros::common::
      CallBackEvent;  //包含了三个数据处理回调函数以及其他回调函数
  ...
  void SetConfiguration(const MappingAndLocationConfig &config);
  ...
  MappingAndLocationConfig GetMappingAndLocationConfig();
 ...
 private:
  ...
  static void ImuCallBackFunc(
      void *object, char *buf,
      const int size);  //-----------------------增加Imu回调函数
  ...
  static void *SendPose(void *ptr);
  std::shared_ptr<DeviceState> device_state_;
  OdometerMessage odom_data_;

  Position radar_pose_base_link_;  //雷达坐标系的位姿，相对于机器人坐标系

  MappingAndLocationConfig connfig_;
  ...
  std::string
      init_pose_file_path;  // 初始位姿文件（机器人关机时侯的位置）存放路径,必须和地图管理模块的地图文件路径一样
