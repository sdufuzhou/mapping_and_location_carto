...
class MappingManagerImpl {
 public:
  ...
  void SetRadarData(const RadarSensoryMessage &data);//---------------就是接收数据----------
  void SetOdomData(const OdometerMessage &data);
  void SetImuData(const ImuSensoryMessage &data);//----------------新加-------------
  void SetDeviceState(
...
//线程互斥锁声明
  ...
  pthread_mutex_t mutex_odom_data;//在多个不同的线程中，读、写都需要加锁，且在线程中声明变量
  ...
  MappingInterface *mp_MappingModule = nullptr;//子类为CartoMapping，多态的应用
