...
void MappingAndLocationConfigTransition(
    const MappingAndLocationCfg& config_input,
    MappingAndLocationConfig& config_result) {
  std::cout << "#######进入MappingAndLocationConfigTransition函数" << std::endl;
  config_result.node_name = config_input.node_name;
  config_result.sub_odom_topic = config_input.sub_odom_topic;
  config_result.sub_radar_topic = config_input.sub_radar_topic;
  config_result.sub_imu_topic = config_input.sub_imu_topic;
  config_result.sub_global_locate_cmd_topic =
      config_input.sub_global_locate_cmd_topic;
  config_result.sub_start_mapping_cmd_topic =
      config_input.sub_start_mapping_cmd_topic;
  config_result.sub_stop_mapping_cmd_topic =
      config_input.sub_stop_mapping_cmd_topic;
  config_result.ad_state_topic = config_input.ad_state_topic;
  config_result.ad_pose_topic = config_input.ad_pose_topic;
  config_result.log_file_name = config_input.log_file_name;
  config_result.log_level = config_input.log_level;
  config_result.is_printf_to_terminal = config_input.is_printf_to_terminal;
  config_result.mapping_algorithm =
      (MappingAlgorithm)config_input.mapping_algorithm;
  // karto_config
  {
    auto& karto_config = config_result.karto_config;
    karto_config.mapping_pattern =
        (MappingPattern)(config_input.karto_config.mapping_pattern);
    karto_config.distance_step = config_input.karto_config.distance_step;
    karto_config.angular_step = config_input.karto_config.angular_step;
    karto_config.mapping_start_angle = config_input.radar_start_angle;
    karto_config.mapping_end_angle = config_input.radar_end_angle;
    karto_config.radar_resolution = config_input.radar_resolution;
    karto_config.mapping_resolution = config_input.radar_resolution;
    karto_config.radar_position_x = config_input.radar_position_x;
    karto_config.radar_position_y = config_input.radar_position_y;
    karto_config.radar_position_theta = config_input.radar_position_theta;
    karto_config.mapping_laser_min_range =
        config_input.karto_config.mapping_laser_min_range;
    karto_config.mapping_laser_max_range =
        config_input.karto_config.mapping_laser_max_range;
    karto_config.travel_distance = config_input.karto_config.distance_step;
    karto_config.travel_angle = config_input.karto_config.angular_step;
    karto_config.loop_search_distance =
        config_input.karto_config.loop_search_distance;
    karto_config.map_data_file_path =
        config_input.karto_config.map_data_file_path;
    karto_config.map_config_file_path =
        config_input.karto_config.map_config_file_path;
  }

  // carto_config
  {
    auto& carto_config = config_result.carto_config;
    carto_config.mapping_pattern = config_input.carto_config.mapping_pattern;
    carto_config.mapping_start_angle = config_input.radar_start_angle;
    carto_config.mapping_end_angle = config_input.radar_end_angle;
    carto_config.mapping_resolution = config_input.radar_resolution;
    carto_config.radar_resolution = config_input.radar_resolution;
    carto_config.radar_position_x = config_input.radar_position_x;
    carto_config.radar_position_y = config_input.radar_position_y;
    carto_config.radar_position_theta = config_input.radar_position_theta;
    carto_config.wheel_odom_position_theta =
        config_input.carto_config.wheel_odom_position_theta;
    carto_config.wheel_odom_position_x =
        config_input.carto_config.wheel_odom_position_x;
    carto_config.wheel_odom_position_y =
        config_input.carto_config.wheel_odom_position_y;
    carto_config.map_config_file_path =
        config_input.carto_config.map_config_file_path;
    carto_config.map_data_file_path =
        config_input.carto_config.map_data_file_path;
    carto_config.num_background_threads =
        config_input.carto_config.num_background_threads;
    ...
    carto_config.collate_by_trajectory =
        config_input.carto_config.collate_by_trajectory;
    // trajectory_config
    {
      auto trajectory_config_in = config_input.carto_config.trajectory_config;
    ...
    // pose_graph_config
    {
      auto pose_graph_config_in = config_input.carto_config.pose_graph_config;
      ...
      // constraint_builder_config
      {
...

        {
          auto fast_correlative_scan_matcher_config_in =
              constraint_builder_config_in.fast_correlative_scan_matcher_config;
        ...
        {
          auto ceres_scan_matcher_config_in =
              constraint_builder_config_in.ceres_scan_matcher_config;
           //外层赋值给下一层ceres_scan_matcher_config_in   
          ceres_scan_matcher_config.occupied_space_weight =
              ceres_scan_matcher_config_in.occupied_space_weight;
          ceres_scan_matcher_config.translation_weight =
              ceres_scan_matcher_config_in.translation_weight;
  ...
  // location_config
  auto location_config_in = config_input.location_config;
  config_result.location_config = {
      config_input.radar_start_angle,
      config_input.radar_end_angle,
      config_input.radar_resolution,
      config_input.radar_position_x,
      config_input.radar_position_y,
      config_input.radar_position_theta,
...
bool ReadMappingAndLocationConfig(const std::string& config_dir,
  ...
  std::string full_name = "./config/mappingandlocation.json";
  std::cout << "#######进入ReadMappingAndLocationConfig函数" << std::endl;
  if (ConfigUtils::decode(full_name.c_str(), temp_config)) {
    std::cout << "#######if判断为真" << std::endl;
...
void DefaultWriteMappingAndLocationConfig(const std::string& config_dir) {
  std::cout << "######进入DefaultWriteMappingAndLocationConfig函数"
            << std::endl;
  system("mkdir -p config");
  MappingAndLocationCfg config_input;
  // 独有的部分（最外层的Xpack直接按下面方式赋值）
  config_input.node_name = "mapping_and_location";
  config_input.sub_odom_topic = "odom_topic";
  config_input.sub_radar_topic = "radar/wj716/sensory";
  config_input.sub_imu_topic = "imu_sensory_msg";
  config_input.sub_global_locate_cmd_topic = "global_locate_cmd";
  config_input.sub_start_mapping_cmd_topic = "start_mapping";
  config_input.sub_stop_mapping_cmd_topic = "stop_mapping";
  config_input.ad_state_topic = "location_state";
  config_input.ad_pose_topic = "location_pose";
  config_input.log_file_name = "mapping_location_log";
  config_input.log_level = 0;
  config_input.is_printf_to_terminal = true;
  config_input.mapping_algorithm = 0;
  config_input.radar_start_angle = -95.0;
  config_input.radar_end_angle = 95.0;
  config_input.radar_resolution = 1.0;
  config_input.radar_position_x = 0.361;
  config_input.radar_position_y = 0.0;
  config_input.radar_position_theta = 0.0;

  {
    auto& karto_config = config_input.karto_config;
    ...
    karto_config.map_data_file_path = "./map";
    karto_config.map_config_file_path = "./config";
  }

  // carto_config
  {
    auto& carto_config = config_input.carto_config;
    ...
    carto_config.map_data_file_path = "./map";
    ...
    // trajectory_config
    {
      ...
      // submap_config
      {
        ...
        // range_data_inserter_config
        {
          ...
          range_data_inserter_config
              .probability_grid_range_data_inserter_config = {true, 0.55, 0.45};
          range_data_inserter_config.tsdf_range_data_inserter_config = {
    ...
    // pose_graph_config
    {
      ...
      // optimization_problem_config
      {
        ...
        optimization_problem_config.fixed_frame_pose_use_tolerant_loss = false;
        optimization_problem_config.fixed_frame_pose_tolerant_loss_param_b = 1;
        optimization_problem_config.fixed_frame_pose_tolerant_loss_param_a = 1;
  ...
  // location_config
  config_input.location_config = {true, 0.0, 19.0, 1.0, 10.0,  10.0,
                                  2,    10,  19.0, 0.5, 0.1,   0.1,
                                  5,    60,  0,    0,   false, "./map"};
  //对赋值后的参数进行编码并写入json文件
  string s = ConfigUtils::ecode(config_input);
