...
OrderedMultiQueue::OrderedMultiQueue() {}  //空构造函数

OrderedMultiQueue::~OrderedMultiQueue() {
  for (auto& entry : queues_) {
    CHECK(entry.second.finished);  //检查
  }
}
...
QueueKey OrderedMultiQueue::GetBlocker() const {
  CHECK(!queues_.empty());//确保至少存在一个队列。
  return blocker_;
}
 ...
 * 退出条件3 数据队列中数据的个数只有1个,又不是完成状态,不能确定状态, 一般为阻塞，就先退出
 */
void OrderedMultiQueue::Dispatch() {
  while (true) {
    ...
    */
    ...
    // Step: 1 遍历所有的数据队列,
    // Step: 1 找到所有数据队列的第一个数据中时间最老的一个数据）
    ...
    // 大多数情况, 数据时间都会超过common_start_time的，这个时间不会被改变，是所有队列的第一个数据中时间戳最晚的时间也就是所有队列的最晚起始时间
...
void OrderedMultiQueue::CannotMakeProgress(const QueueKey& queue_key) {
  ...
  for (auto& entry : queues_) {
    ...
    if (entry.second.queue.Size() > kMaxQueueSize) {
      ...
      // data: (0, points2) [ WARN] [1628516439.089736487,
      // 1606808659.869309184]: W0809 21:40:39.000000 10662
      // ordered_multi_queue.cc:230] Queue waiting for data: (0, points2)
      return;
    }
  }
}
