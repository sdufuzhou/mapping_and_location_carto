...
PointCloud AdaptivelyVoxelFiltered(
  ...
  // param: adaptive_voxel_filter.min_num_points 满足小于等于这个值的点云个数满足要求, 足够稀疏，就没有必要再进行体素滤波了
  if (point_cloud.size() <= options.min_num_points()) {
    // 'point_cloud' is already sparse enough.
    return point_cloud;
  }
  // param: adaptive_voxel_filter.max_length（体素滤波使用的立方体的最大边长） 进行一次体素滤波
  PointCloud result = VoxelFilter(point_cloud, options.max_length());
  // 如果按照最大边长进行体素滤波之后还超过这个数了, 就说明已经是这个参数下最稀疏的状态了, 直接返回
  /*如果体素滤波边长设置的过小，会导致扫描匹配不准确，边长过大会使不满足“滤波后点云的个数不能小于这个最小数量”*/
  if (result.size() >= options.min_num_points()) {/*滤波后点云的个数不能小于这个最小数量*/
  ...
  for (float high_length = options.max_length();
    ...
    if (result.size() >= options.min_num_points()) {
      ...
      // 以二分查找的方式找到合适的过滤边长, 当边缘长度最多减少 10% 时停止，此时的mid_length就是最合适的自适应体素滤波的边长
      while ((high_length - low_length) / low_length > 1e-1f) {
        const float mid_length = (low_length + high_length) / 2.f;/*mid_length：先设置一个中间值*/
        ...
        if (candidate.size() >= options.min_num_points()) {
          low_length = mid_length;/*由计算公式来看，mid_length永远大于low_length*/
          result = candidate;
        } 
...
std::vector<bool> RandomizedVoxelFilterIndices(
  ...
  for (size_t i = 0; i < point_cloud.size(); i++) {
    // 获取VoxelKeyType对应的value（std::pair<int, int>>）的引用
...
PointCloud AdaptiveVoxelFilter(
  ...
  return AdaptivelyVoxelFiltered(
      ...
      // FilterByMaxRange()函数相当于对点云又进行了一次距离的滤波，returns的距离不超过30
}
