...
CollatedTrajectoryBuilder::CollatedTrajectoryBuilder(
    ...
    : sensor_collator_(sensor_collator),//初始化参数列表并不一定只初始构造函数用到的参数，而且初始化的参数，函数内部也未必用上
  ...
  absl::flat_hash_set<std::string> expected_sensor_id_strings;//所有话题的名字构建一个集合
  ...
  // sensor::Collator的初始化，调用 sensor_collator_->AddTrajectory() 函数，实际就是对 sensor::Collator 实例对象的初始化
  sensor_collator_->AddTrajectory(
      trajectory_id, expected_sensor_id_strings,
      [this](const std::string& sensor_id, std::unique_ptr<sensor::Data> data) {
        HandleCollatedSensorData(sensor_id, std::move(data));
        //该 lambda 函数体，比较简单，实际就是调用了 CollatedTrajectoryBuilder::HandleCollatedSensorData() 函数
      });
}
...
void CollatedTrajectoryBuilder::HandleCollatedSensorData(
  ...
  if (it == rate_timers_.end()) {
    ...
    // emplace().second表示插入成功,只有key在map中不存在时才插入成功
    it = rate_timers_
             .emplace(
  ...
  // +/- 4.35e-05 s (pulsed at 100.44% real time) [ INFO]:
  // collated_trajectory_builder.cc:72] scan rate: 19.83 Hz 5.04e-02 s
  // +/- 4.27e-05 s (pulsed at 99.82% real time)

  // 将排序好的数据送入 GlobalTrajectoryBuilder中的AddSensorData()函数中进行使用，见dispatchable.h文件
