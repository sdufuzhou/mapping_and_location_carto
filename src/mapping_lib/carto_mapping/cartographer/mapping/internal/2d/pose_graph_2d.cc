 ...
 * @param[in] trajectory_id 轨迹id
 * @param[in] insertion_submaps 子地图 active_submaps（只有两个）
...
// 将 landmark数据加入到data_.landmark_nodes中 这个任务放入到任务队列中
...
void PoseGraph2D::HandleWorkQueue(
    const constraints::ConstraintBuilder2D::Result& result) {
  {
    ...
    data_.constraints.insert(data_.constraints.end(), result.begin(),
                             result.end());//第一个参数为添加的位置，第2、3个参数代表添加从result第一个数据到最后一个数据
  }
...
void PoseGraph2D::RunOptimization() {
  ...
  for (const int trajectory_id : node_data.trajectory_ids()) {
    ...
    const transform::Rigid3d old_global_to_new_global =
        local_to_new_global * local_to_old_global.inverse();//xA=B?
    // 这一次优化的node的最后一个id
