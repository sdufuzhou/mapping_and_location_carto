...
class OccupiedSpaceCostFunction2D {
 public:
  ...
  bool operator()(const T* const pose, T* residual) const {
    Eigen::Matrix<T, 2, 1> translation(pose[0], pose[1]);//pose[0]、pose[1]存放的是x,y值
    Eigen::Rotation2D<T> rotation(pose[2]);//pose[2]里存放的角度值
    Eigen::Matrix<T, 2, 2> rotation_matrix = rotation.toRotationMatrix();//旋转通过函数toRotationMatrix直接转化为矩阵的形式
    Eigen::Matrix<T, 3, 3> transform;
    transform << rotation_matrix, translation, T(0.), T(0.), T(1.);//构建一个3*3的变换矩阵
    ...
    for (size_t i = 0; i < point_cloud_.size(); ++i) {
      ...
      const Eigen::Matrix<T, 3, 1> point((T(point_cloud_[i].position.x())),
                                         ...
                                         T(1.));
      // 根据预测位姿对单个点进行坐标变换
      const Eigen::Matrix<T, 3, 1> world = transform * point;//把围绕local坐标系原点的点，变换到世界坐标系下
      // 获取三次插值之后的栅格free值, Evaluate函数内部调用了GetValue函数
      interpolator.Evaluate(
          (limits.max().x() - world[0]) / limits.resolution() - 0.5 +
              static_cast<double>(kPadding),
          (limits.max().y() - world[1]) / limits.resolution() - 0.5 +
              static_cast<double>(kPadding),//计算的是x,y两个方向上的像素坐标
          &residual[i]);
      // free值越小（cartographer里存的是空闲的值）, 表示占用的概率越大
