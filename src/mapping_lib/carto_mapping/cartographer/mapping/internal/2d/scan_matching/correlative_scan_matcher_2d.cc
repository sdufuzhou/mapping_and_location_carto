...
SearchParameters::SearchParameters(const double linear_search_window,
  ...
  float max_scan_range = 3.f * resolution;//定义三倍分辨率的一个数据
  ...
  // 根据论文里的公式 求得角度分辨率 angular_perturbation_step_size
    /*通过仔细选择步长来提高效率。我们选择角阶长δ θ
    保证扫描点在最大范围d最大不移动超过r（宽度一个像素）
  */
  const double kSafetyMargin = 1. - 1e-3;//定义一个比1稍微小点的值
  angular_perturbation_step_size =
      kSafetyMargin * std::acos(1. - common::Pow2(resolution) /
                                         (2. * common::Pow2(max_scan_range)));//  Pow2代表平方

  // 范围除以分辨率得到个数
  num_angular_perturbations =
      std::ceil(angular_search_window / angular_perturbation_step_size);
  // num_scans是要生成旋转点云的个数, 将 num_angular_perturbations 扩大了2倍，变相的使角度搜索窗口从20度变为40度
  ...
  // linear_bounds 的作用是确定每一个点云的最大最小边界（每一个点云的最大最小边界都相同）
...
std::vector<sensor::PointCloud> GenerateRotatedScans(
  ...
  // 起始角度(注意这里前面扩大了2倍，相当于2x的起始为0，这里变成了-x到+x,总个数不变,依然为2x，但生成的点云的个数为2x+1个)
  ...
  for (int scan_index = 0; scan_index < search_parameters.num_scans;
    ...
    // 将 point_cloud 绕Z轴旋转了delta_theta，并放入容器中
...
std::vector<DiscreteScan2D> DiscretizeScans(
  ...
  for (const sensor::PointCloud& scan : scans) {
    // 将discrete_scans中的每一个 DiscreteScan2D（也是一个vector容器） 的size设置为这一帧点云中所有点的个数
    discrete_scans.emplace_back();//和push_back的功能相同，都是在vector容器尾部插入一个元素，这里插入的为空
    ...
    for (const sensor::RangefinderPoint& point : scan) {
      ...
      const Eigen::Vector2f translated_point =
          Eigen::Affine2f(initial_translation) * point.position.head<2>();// Eigen::Affine2f仿射变换
      //point.position.head<2>()代表取出这个点的x，y坐标


      // 将平移后的点 对应的栅格坐标放入discrete_scans（先判断平移后的点落在哪个栅格上）
