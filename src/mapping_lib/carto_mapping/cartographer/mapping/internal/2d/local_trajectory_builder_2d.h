...
class LocalTrajectoryBuilder2D {
 public:
  ...
  struct InsertionResult {
    std::shared_ptr<const TrajectoryNode::Data>
        constant_data;  // constant恒定的，不变的,持续不断的
 ...
 private:
  ...
  MotionFilter motion_filter_; //对运动进行过滤，如果运动距离或时间太短，则不进行处理
  scan_matching::RealTimeCorrelativeScanMatcher2D
      real_time_correlative_scan_matcher_;//实时的2D扫描匹配器
  scan_matching::CeresScanMatcher2D ceres_scan_matcher_;//ceres的扫描匹配器 

  std::unique_ptr<PoseExtrapolator> extrapolator_;//位姿估计器

  int num_accumulated_ = 0;  //代表的雷达数据的帧数
  ...
  RangeDataCollator range_data_collator_; //对雷达数据（假设有多个相同类型的传感器，以两个雷达为例）进行时间同步的类
};
