...
class GlobalTrajectoryBuilder : public mapping::TrajectoryBuilderInterface {
 public:
   ...
   * @param[in] local_slam_result_callback 前端的回调函数，保存前端slam的结果
   * @param[in] pose_graph_odometry_motion_filter 运动滤波器；里程计过滤器，没有启用
   */
  ...
  void AddSensorData(
    ...
    CHECK(local_trajectory_builder_)
        << "Cannot add TimedPointCloudData without a LocalTrajectoryBuilder.";//检测是否存在前端，没有前端则报错
  uint64_t t1 = gomros::common::GetCurrentTime_us(); 
    // 进行扫描匹配, 返回匹配后的结果（进行局部的子图匹配）
...
LOG_DEBUG << "扫描匹配耗时"<< t2 - t1;
    ...
    // LocalTrajectoryBuilder2D::InsertionResult 如果雷达成功插入到地图中
    ...
    // 将结果数据传入回调函数中（必须是有值的）,
    // 进行保存(子图、位姿、坐标、时间、轨迹id、位姿纠正后的雷达数据)
  ...
  // 一个是进入后端pose_graph_ 加入到后端之前, 先做一个距离的计算,
  // 只有时间,移动距离,角度 变换量大于阈值才加入到后端中
  void AddSensorData(const std::string& sensor_id,
