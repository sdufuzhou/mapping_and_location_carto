...
//翻译：
/* 使用 sensor::CollatorInterface 整理传感器数据（进行数据的分发）,
然后将其传递到2D和3D通用的mapping::TrajectoryBuilderInterface，interface：界面，接口*/

// 处理传感器数据, 使其按照时间排列, 然后传入GlobalTrajectoryBuilder（也是TrajectoryBuilderInterface的子类）
/*下面利用父类的指针指向子类的对象，利用C++的多态技术，同一个父类对应多个子类*/
class CollatedTrajectoryBuilder : public TrajectoryBuilderInterface {
 public:
  ...
  父类的析构函数如果加了virtual，子类的析构函数可以不加*/
  ...
  void AddSensorData(
    ...
    //override关键字表示重写，也就是说该函数就是重写父类函数，而不是其他自身构建的函数，
  //如果该函数传入的参数或者返回值与父类不一致，则会报错
  }
  ...
  void AddSensorData(
    ...
    /*wrapped_trajectory_builder_为Global_trajectory_builder_*/
  }
  ...
  void AddSensorData(const std::string& sensor_id,
    ...
    if (collate_landmarks_) {
      AddData(sensor::MakeDispatchable(sensor_id, landmark_data));//添加到阻塞队列,对数据按时间排序之后再分发
      return;
    }
    wrapped_trajectory_builder_->AddSensorData(sensor_id, landmark_data);
    //则数据直接通过 wrapped_trajectory_builder_->AddSensorData() 添加到 GlobalTrajectoryBuilder 之中

    //collate_fixed_frame_，collate_landmarks_
    //就是控制 GPS 与 landmark 数据是否添加到阻塞队列，然后排序之后再分发给 GlobalTrajectoryBuilder
  }
