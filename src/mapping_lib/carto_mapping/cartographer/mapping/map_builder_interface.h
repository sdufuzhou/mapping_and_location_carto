...
class MapBuilderInterface { /*是一个基类，不能实例化对象，只能通过指针的方式*/
 public:
  using LocalSlamResultCallback =
      TrajectoryBuilderInterface::LocalSlamResultCallback;
  /*using：C++11里的起别名操作（免得大代码太过于臃肿）：
  例：对TrajectoryBuilderInterface::LocalSlamResultCallback起别名
  叫做LocalSlamResultCallback
  */
  using SensorId = TrajectoryBuilderInterface::SensorId;

  MapBuilderInterface() {}
  virtual ~MapBuilderInterface() {} /*基类的析构函数前要加virtual*/

  MapBuilderInterface(const MapBuilderInterface&) =
      delete; /*禁止编译器自动生成这两个函数*/
  ...
  virtual int AddTrajectoryBuilder(
      ...
      LocalSlamResultCallback local_slam_result_callback) = 0; /*纯虚函数*/
