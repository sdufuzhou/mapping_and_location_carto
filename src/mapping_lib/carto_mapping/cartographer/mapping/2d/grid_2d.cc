...
Grid2D::Grid2D(const MapLimits& limits, float min_correspondence_cost,
    ...
    : limits_(limits),                               //    ：后的操作：代表对一些参数进行初始化列表
      correspondence_cost_cells_(
          limits_.cell_limits().num_x_cells * limits_.cell_limits().num_y_cells,
          kUnknownCorrespondenceValue),  // 初始化所有栅格值为0，地图大小：x*y个cell
...
void Grid2D::FinishUpdate() {
  while (!update_indices_.empty()) {
    ...
    correspondence_cost_cells_[update_indices_.back()] -= kUpdateMarker;//更新值
    update_indices_.pop_back();//更新索引
  }
}
...
void Grid2D::ComputeCroppedLimits(Eigen::Array2i* const offset,
  ...
  if (known_cells_box_.isEmpty()) {
    *offset = Eigen::Array2i::Zero();//初始化
    *limits = CellLimits(1, 1);//初始化
    return;
  }

  //不为空
  *offset = known_cells_box_.min().array();//返回bounding_box的最小像素坐标
...
void Grid2D::GrowLimits(const Eigen::Vector2f& point) {
  GrowLimits(point, {mutable_correspondence_cost_cells()},//mutable代表可以修改的
             {kUnknownCorrespondenceValue});//用花括号作初始化代表传入的是一个vector
}
