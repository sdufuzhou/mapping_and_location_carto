...
void Submap2D::Finish() {
  ...
  // 将子图标记为完成状态
  set_insertion_finished(true);//初始为false
}
...
std::vector<std::shared_ptr<const Submap2D>> ActiveSubmaps2D::InsertRangeData(
  ...
  if (submaps_.empty() ||
    ...
    AddSubmap(range_data.origin.head<2>());//.head代表传入的是origin坐标的x,y值
  }
  // 将一帧雷达数据同时写入两个子图中
  for (auto& submap : submaps_) {
    submap->InsertRangeData(range_data, range_data_inserter_.get());//.get()代表获取range_data_inserter_的原始指针
  }
      ...
      return absl::make_unique<ProbabilityGrid>(
          MapLimits(resolution,
                    ...
                    origin.cast<double>() + 0.5 * kInitialSubmapSize *
                                                resolution *
                                                Eigen::Vector2d::Ones(),//把物理坐标转换为向量
