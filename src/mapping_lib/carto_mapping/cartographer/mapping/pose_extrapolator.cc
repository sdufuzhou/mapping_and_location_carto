...
namespace mapping {
/*Cartographer中的位姿推测器融合里程计数据和IMU数据，为Local SLAM提供初始位姿估计*/

/**
 * @brief 构造函数
 * 
 * @param[in] pose_queue_duration 记录了位姿之间的时间间隔0.001
 * @param[in] imu_gravity_time_constant 记录了重力加速度10
 */
...
// 使用imu数据进行PoseExtrapolator的初始化（非关键函数）
...
void PoseExtrapolator::AddPose(const common::Time time, 
  ...
  if (imu_tracker_ == nullptr) {
    ...
    // imu_tracker_的初始化，tracker翻译为：跟踪器
    imu_tracker_ =
  ...
  // timed_pose_queue_最少是2个数据并且保持pose队列中第二个pose的时间要大于 time - pose_queue_duration_：
  while (timed_pose_queue_.size() > 2 &&timed_pose_queue_[1].time <= time - pose_queue_duration_) 
  {
    timed_pose_queue_.pop_front();//timed_pose_queue_数据队列里的数据的修剪
  }
...
/** 
 * @brief 添加里程计数据到指定容器，并更新成员变量angular_velocity_from_odometry_、linear_velocity_from_odometry_
 * @param odometry_data 记录了里程计数据
 */

// 向odom数据队列中添加odom数据,并进行数据队列的修剪,并计算角速度与线速度
void PoseExtrapolator::AddOdometryData(
    const sensor::OdometryData& odometry_data) {
  // 判定timed_pose_queue_非空，并且里程计的时间戳不小于timed_pose_queue_中最后一个位姿对应的时间戳      
  CHECK(timed_pose_queue_.empty() ||
  ...
  // 取最新与最老的两个里程计数据
  const sensor::OdometryData& odometry_data_oldest = odometry_data_.front();//容器的第一个数据为最先插入的数据，所以为最老的数据
  const sensor::OdometryData& odometry_data_newest = odometry_data_.back();
  // 最新与最老odom数据间的时间差（获得两个位姿的时间差）
  const double odometry_time_delta =
      common::ToSeconds(odometry_data_oldest.time - odometry_data_newest.time);
  // 计算两个位姿间的坐标变换（ 计算位姿偏移）
  const transform::Rigid3d odometry_pose_delta =
      odometry_data_newest.pose.inverse() * odometry_data_oldest.pose;
  //就相当解矩阵方程：Ax=B，x可以表示为A到B的坐标变换
  //A^-1*Ax=  A^-1*B，x=A^-1*B， odometry_data_newest.pose到 odometry_data_oldest.pose的变换

/*假设都是匀速模型*/

  // 两个位姿间的旋转量(角度)除以时间得到 tracking frame 的角速度
  ...
  // 平移量除以时间得到 tracking frame 的线速度, 只在x方向有数值（机器人的平移是朝着某个固定的方向的）
  const Eigen::Vector3d
      linear_velocity_in_tracking_frame_at_newest_odometry_time =
          odometry_pose_delta.translation() / odometry_time_delta;

  /*根据位姿队列中最后一个位姿的姿态    乘以    上次添加位姿时的姿态 预测到time时刻的姿态变化量

得到预测的 最新里程计数据时刻 tracking frame 在 local 坐标系下的姿态*/

  /*orientation_at_newest_odometry_time：计算机器人最新的方向信息，记录在这个变量里*/
  const Eigen::Quaterniond orientation_at_newest_odometry_time =
      timed_pose_queue_.back().pose.rotation() *
      ExtrapolateRotation(odometry_data_newest.time,//ExtrapolateRotation：预测旋转的函数
...
transform::Rigid3d PoseExtrapolator::ExtrapolatePose(const common::Time time) {
  ...
  if (cached_extrapolated_pose_.time != time) {
    // 预测tracking frame在local坐标系下time时刻的位置（平移是坐标，旋转是姿态）
    ...
    cached_extrapolated_pose_ =TimedPose{time, transform::Rigid3d{translation, rotation}};//赋值
  }
  return cached_extrapolated_pose_.pose;//.pose得到的就是位姿（包含位置和姿态）
}
...
Eigen::Quaterniond PoseExtrapolator::EstimateGravityOrientation(
  ...
  // 返回 imu_tracker 预测到的time时刻的姿态
  return imu_tracker.orientation();//orientation函数来计算姿态
}
...
void PoseExtrapolator::UpdateVelocitiesFromPoses() {
  ...
  // 平移量除以时间得到 tracking frame 在 local坐标系下的线速度（传入的pose本来就是相对于local坐标系的）
  linear_velocity_from_poses_ =
      (newest_pose.translation() - oldest_pose.translation()) / queue_delta;
  // 角度变化量除以时间得到角速度，得到 tracking frame 在 local坐标系下的角速度
  angular_velocity_from_poses_ =
      transform::RotationQuaternionToAngleAxisVector(
          oldest_pose.rotation().inverse() * newest_pose.rotation()) /
      queue_delta;
      //解矩阵方程：Ax=B，x可以表示为A到B的坐标变换
      //A^-1*Ax=  A^-1*B，x=A^-1*B，即oldest_pose.rotation()到 newest_pose.rotation()的变换关系（用四元数表示）
      //后面通过RotationQuaternionToAngleAxisVector（）这个函数把四元数得到一个旋转向量，旋转向量中包含一个角度值
}

// 修剪imu的数据队列,丢掉过时的imu数据
void PoseExtrapolator::TrimImuData() {
  // 保持imu队列中第二个数据(Imu数据的个数最少是一个)的时间要大于最后一个位姿的时间，否则要舍弃掉
...
void PoseExtrapolator::AdvanceImuTracker(const common::Time time,
  ...
  while (it != imu_data_.end() && it->time < time) {
    // 先预测出当前时刻的姿态与重力方向，后面再校准
    imu_tracker->Advance(it->time);
    // 通过添加线速度的观测,更新重力的方向,并根据重力的方向对上一时刻预测的姿态进行校准
...
Eigen::Quaterniond PoseExtrapolator::ExtrapolateRotation(
  ...
  // 求取上一帧到当前时刻预测出的姿态变化量：上一帧姿态四元数的逆乘以当前时刻预测出来的姿态四元数
...
Eigen::Vector3d PoseExtrapolator::ExtrapolateTranslation(common::Time time) {
  ...
  if (odometry_data_.size() < 2) {
    return extrapolation_delta * linear_velocity_from_poses_;  // 如果不使用里程计就使用通过pose计算出的线速度
  }
