...
/**
  ...
  这时候生产者白白浪费大好时光。 而使用这种模式之后, 生产者将数据丢到缓冲区,
  继续生产, 完全不依赖消费者, 程序执行效率会大大提高。

  复用：通过将生产者类和消费者类独立开来,
  可以对生产者类和消费者类进行独立的复用与扩展
 */

template <typename T>//模板参数就是 std::deque 队列存储的数据类类型
class BlockingQueue {
 public:
  ...
  const R* Peek() {
    ...
    return deque_.front().get();  //获取原始的指针
  }
