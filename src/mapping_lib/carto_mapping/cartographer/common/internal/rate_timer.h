...
class RateTimer {
 public:
  ...
  // time.
  // c++11:
  /*explicit关键字：只能用于修饰只有一个参数的类构造函数；
  作用就是防止类构造函数的隐式自动转换*/
  ...
  // 返回脉搏率（提供时间）与wall time率的比率，就是传感器数据的利用率
  // 例如, 如果传感器产生 10 Hz 的脉冲, 但我们将 Pulse(脉搏，脉冲) 称为 20 Hz
  // 墙时间, 这将返回 2
  ...
  void Pulse(common::Time time) {
    ...
    while (events_.size() > 2 &&
           (events_.back().wall_time - events_.front().wall_time) >
               window_duration_) {
      events_.pop_front();
      //最终 events_.back().wall_time - events_.front().wall_time 表示15秒内调用数据的首尾差
    }
  }
