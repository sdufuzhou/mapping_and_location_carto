...
ThreadPool::ThreadPool(int num_threads) {
  ...
  /*传入的数量小于零就报错*/
  absl::MutexLock locker(&mutex_); /*上锁*/
  for (int i = 0; i != num_threads; ++i) {
    pool_.emplace_back([this]() { ThreadPool::DoWork(); });
    /*传入几个num_thread，就生成几个pool容器，对应几个线程，执行几次DoWork函数（死循环）*/
    /*C++ 11对容器的push_back, push_front, insert 增加了新的用法，
    与之对应的是emplace_back，emplace_front, emplace.
    它们的作用是在操作容器时可以调用对应类型的构造数，*/
  }
}
