...
class ConfigurationFileResolver : public FileResolver {
 public:
  ...
  explicit ConfigurationFileResolver(
      ...
      //比如在一个构造函数中，其期待传入的数据类型并非 int 类型，
      //而用户误传入了一个 int 10 进去，其就有可能发生隐式转换，比如编译器把 int 10 封装成一个类对象传入

  std::string GetFullPathOrDie(const std::string& basename) override;//成员函数就是接口
