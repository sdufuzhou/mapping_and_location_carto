...
#include "include/mapping_lib/karto_mapping.h"
...
KartoMapping::KartoMapping(const KartoMappingConfig &config,
  ...
  laser_min_angle_ = m_MappingConfig.mapping_start_angle / 180 * M_PI;//转换为弧度
  laser_max_angle_ = m_MappingConfig.mapping_end_angle / 180 * M_PI;
  laser_resolution_ = m_MappingConfig.laser_resolution / 180 * M_PI;
...
void KartoMapping::SetConfiguration(void *config) {
  m_MappingConfig = *(reinterpret_cast<KartoMappingConfig *>(config));
  laser_min_angle_ = m_MappingConfig.mapping_start_angle / 180 * M_PI;
  laser_max_angle_ = m_MappingConfig.mapping_end_angle / 180 * M_PI;
  laser_resolution_ = m_MappingConfig.laser_resolution / 180 * M_PI;
  laser_max_range_ = m_MappingConfig.mapping_laser_max_range;
  laser_min_range_ = m_MappingConfig.mapping_laser_min_range;
  ...
  if (m_MappingConfig.mapping_pattern == Offline) {
    ...
    if (first_laser_) {
      ...
      if (laser_min_angle_ <
        ...
        ERROR_LOG(mp_logger, "建图模块配置文件雷达最小使用角度小于雷达数据最小角度\n");
      }
      if (laser_max_angle_ >
        ...
        ERROR_LOG(mp_logger, "建图模块配置文件雷达最大使用角度大于雷达数据最大角度\n");
      }
      ...
      if (laser_min_range_ <
        ...
        ERROR_LOG(mp_logger, "建图模块配置文件雷达最小使用距离小于雷达数据最小距离\n");
      }
      if (laser_max_range_ >
        ...
        ERROR_LOG(mp_logger, "建图模块配置文件雷达最大使用距离大于雷达数据最大距离\n");
      }
      DEBUG_LOG(mp_logger, "建图模块配置雷达角度分辨率为%f, 起始角度为%f, 终止角度为%f, 最小距离为%f, 最大距离为%f\n",
      laser_resolution_, laser_min_angle_, laser_max_angle_, laser_min_range_, laser_max_range_);
    }
    if (record_data_) {
...
void KartoMapping::StartMapping() {
  ...
  //这段代码的作用是在指定路径下创建一个名为 "map.rawmap" 的空文件，并将其内容设置为一个空格。
  std::string str = " ";
  std::ofstream ofs("./map/map.rawmap");  //创建一个输出文件流对象 ofs，并打开名为 "./map/map.rawmap" 的文件。
  //如果文件不存在，则会创建新文件；如果文件已存在，则会清空文件内容。
  ofs << str;//将空字符串 str 写入文件 ofs，即写入空格
  ofs.close();//关闭文件流 ofs


  system("mkdir -p  map");//命令用于创建一个名为 "map" 的目录。mkdir -p 命令的作用是如果目录已存在，则不做任何操作；
  //如果目录不存在，则创建该目录。这里使用 -p 参数是为了确保在创建目录时能够创建所有需要的上级目录
  ...
  start_mapping_ = true;
  DEBUG_LOG(mp_logger, "创建存储数据线程\n");
  //并启动一个线程来记录数据。
  pthread_create(&record_data_thread_, NULL, RecordDataFunc, this);
}

bool KartoMapping::IsFinishMapping() {
  return finish_mapping_;
}
...
void KartoMapping::StopMapping(std::string map_name) {
  ...
  // my_karto->SetLaserParam(laser_min_angle_, laser_max_angle_, laser_min_range_,
  //                         laser_max_range_, laser_resolution_);
  // my_karto->begin_slam(&map_data1, map_name);
  // usleep(30000);
  // if (!my_karto->map_successed) {
  //   ERROR_LOG(mp_logger, "Karto 离线建图失败！\n");
  // }
  // delete (my_karto);
}
...
void* KartoMapping::RecordDataFunc(void* ptr) {
  ...
  while(!p->m_RidarList.empty() || p->record_data_ ) {
     ...
     // 使用互斥锁保护姿态列表的操作
        ...
        fprintf(p->m_pMapRawDataFile, "%d\t%ld\t", record_index,odom_data.mlTimestamp);
        int start_i = fabs(p->laser_min_angle_- 
                      lidar_data.mstruRadarMessage.mstruRadarHeaderData.mfAngleMin) /
                    p->angle_increment_;
        int record_cnt =
            fabs((p->laser_max_angle_ - p->laser_min_angle_) / p->laser_resolution_);
        fprintf(p->m_pMapRawDataFile, "%f\t%f\t%f\t%d\t", odom_data.mfX,
                odom_data.mfY, odom_data.mfTheta, record_cnt);
        record_cnt *= p->stepIncreament_;
         // 遍历雷达数据点并写入文件
        for (int i = start_i; i <= (start_i + record_cnt); i += p->stepIncreament_) {
          float xp = lidar_data.mstruRadarMessage.mstruSingleLayerData.mvPoints[i](0);
          float yp = lidar_data.mstruRadarMessage.mstruSingleLayerData.mvPoints[i](1);
...
void* KartoMapping::MappingFunc(void* ptr) {
  ...
  // 设置激光参数
  my_karto->SetLaserParam(p->laser_min_angle_, p->laser_max_angle_, p->laser_min_range_,
                          p->laser_max_range_, p->laser_resolution_);
...
// 如果离线建图失败，则输出错误日志
  if (!my_karto->map_successed) {
    ERROR_LOG(p->mp_logger, "Karto 离线建图失败！\n");
  }
  delete (my_karto);
  // 更新标志位，表示离线建图已完成
...
void KartoMapping::AddOdomDataRaw(const Position &pose_change) {
  ...
  if (timestamp < m_MappingOdom.mlTimestamp) {
    // WARN_LOG(mp_logger, "当前里程计数据时间戳小于最新里程计位姿时间戳\n");
  }
  pthread_mutex_lock(&mutex_odom_list_raw);
