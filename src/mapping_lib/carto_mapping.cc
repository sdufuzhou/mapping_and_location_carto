  ...
  namespace data_process
  {
    namespace mapping_and_location
    {
      ...
      CartoMapping::CartoMapping(const CartoMappingConfig &config)
      {
        ...
        config_ = config;
        // 传感器选择
        use_odom_based = config_.use_odom_data;
        use_imu_based = config_.use_imu_data;

        // 建图模式选择
        offline_mapping = config_.mapping_pattern;
        ...
        pthread_mutex_init(&mutex_ridar_list, nullptr);
        pthread_mutex_init(&mutex_odom_list, nullptr);
      }
      CartoMapping::~CartoMapping()
      {
        pthread_mutex_destroy(&mutex_ridar_list);
      ...
      void CartoMapping::StartMapping()
      {
        ...
        add_data_ = true;
        first_laser_ = true;
        // 初始化 维持的建图里程计的初始位姿以及时间戳
        m_MappingOdom.mlTimestamp = 0;
        m_MappingOdom.mfX = 0;
        m_MappingOdom.mfY = 0;
      ...
      void CartoMapping::StopMapping(std::string map_name)
      {
        ...
        th.detach();
        // pthread_create(&finish_mapping_thread_, NULL, FinishMapping, this);
      }
      void CartoMapping::FinishMapping()
      {
        // SLAM_DEBUG("等待结束建图\n");
        while (!handle_radar_finish_)
        {
          SLAM_INFO("激光雷达数据处理handle_laser未结束,请等待\n");
          usleep(3000000);
        }
        SLAM_DEBUG("开始结束建图\n");
        ...
        if (offline_mapping)
        {
          ...
          SLAM_DEBUG("odom zise %d,radar size %d\n", m_OdomList_inRead.size(),
                     m_RidarList_inRead.size());

          while (!m_OdomList_inRead.empty() || !m_RidarList_inRead.empty())
          {
            if (!m_OdomList_inRead.empty() && !m_RidarList_inRead.empty())
            {
              if (m_OdomList_inRead.front().mlTimestamp <=
                  m_RidarList_inRead.front()
                      .mstruRadarMessage.mstruRadarHeaderData.mlTimeStamp)
              {
                SLAM_WARN("里程计数据较早\n");
                std::thread th(std::bind(&CartoMapping::OdomDataConversionAndAddFunc,
                                         this, m_OdomList_inRead.front()));
                // 线程阻塞，等待线程结束后程序再往下运行
                th.join();
                m_OdomList_inRead.pop_front();
                LOG_WARN << "剩余" << m_OdomList_inRead.size() << "帧里程计数据";
              }
              else
              {
                SLAM_WARN("雷达数据较早\n");
                std::thread th(std::bind(&CartoMapping::LaserDataConversionAndAddFunc,
                                         this, m_RidarList_inRead.front()));
                th.join();
                m_RidarList_inRead.pop_front();
                LOG_WARN << "剩余" << m_RidarList_inRead.size() << "帧雷达数据";
              }
            }
            else if (!m_OdomList_inRead.empty())
            {
              std::thread th(std::bind(&CartoMapping::OdomDataConversionAndAddFunc,
                                       this, m_OdomList_inRead.front()));
              th.join();
            ...
            else if (!m_RidarList_inRead.empty())
            {
              std::thread th(std::bind(&CartoMapping::LaserDataConversionAndAddFunc,
                                       this, m_RidarList_inRead.front()));
              th.join();
        ...
        m_RidarList_inRead.clear();
        m_OdomList_inRead.clear();
      ...
      void CartoMapping::HandleTimeQueue()
      {
        ...
        while (!time_queue.empty() || add_data_)
        {
          auto time_item = *time_queue.begin();
          time_queue.erase(time_queue.begin());
          locker.unlock();
          if (time_item.second == SensorType::Lidar)
          {
            std::unique_lock<std::mutex> lidar_locker(raw_lidar_list_mtx_);
            RadarSensoryMessage laser_data = m_RidarList.front();
            m_RidarList.pop_front();
            lidar_locker.unlock();
            if (!offline_mapping)
            {
          ...
          else if (time_item.second == SensorType::Odom)
          {
            ...
            OdometerMessage odom_data = m_OdomList.front();
          ...
          else
          {
            ...
            ImuSensoryMessage imu_data = m_ImuList.front();
            m_ImuList.pop_front();
            // imu解锁
            imu_locker.unlock();
            if (!offline_mapping)
      ...
      void CartoMapping::SaveLaserDataToFile(RadarSensoryMessage &Laser_data)
      {
        ...
        fprintf(RadarDataFile_, "%d %d %ld %f %f %f %f", record_index_radar,
      ...
      void CartoMapping::SaveOdomDataToFile(Position &pose)
      {
        ...
        //  写入index、时间戳
        fprintf(OdomDataFile_, "%d %ld ", record_index_odom, pose.mlTimestamp);
        //------写入x，y，theta------
      ...
      {
        // 写入index、时间戳
        fprintf(ImuDataFile_, "%d %ld ", record_index_imu, imu_data.time_stamp);
      ...
      void *CartoMapping::HandleLaser(void *ptr)
      {
        ...
        while (!p->m_RidarList.empty() || p->add_data_)
        {
          ...
          { // 保证有数据才可以拿
            ...
            // pthread_mutex_lock(&p->raw_lidar_list_mtx_);
            RadarSensoryMessage Laser_data = p->m_RidarList.front();
            // 取数据，取数据的速度可能比往list中放快
            p->m_RidarList.pop_front(); // 是为了跳出循环
            // pthread_mutex_unlock(&p->mutex_ridar_list_raw);
            locker.unlock();
            // 在线
            if (!p->offline_mapping)
            {
      ...
      void *CartoMapping::HandleOdom(void *ptr)
      {
        ...
        int record_index = 0;
        Position Wheel_Odom;
        while (!p->m_OdomList.empty() || p->add_data_)
        {
          if (!p->m_OdomList.empty())
          {
            // 使用互斥锁保护里程计数据列表的操作
            // pthread_mutex_lock(&p->mutex_odom_list_raw);
            OdometerMessage odom_data = p->m_OdomList.front();
            p->m_OdomList.pop_front();
            // pthread_mutex_unlock(&p->mutex_odom_list_raw);
            ...
            Position pose(odom_data.mclDeltaPosition.mlTimestamp,
                          ...
                          atan2(local_to_tracking(1, 0), local_to_tracking(0, 0)));
            // 在线
            if (!p->offline_mapping && p->use_odom_based)
            {
              // 封装成一个函数
      ...
      void CartoMapping::OdomDataConversionAndAddFunc(Position &pose)
      {
        ...
        // pthread_mutex_lock(&paint_mutex);
        // if (latest_odom_timestamp < 0 || latest_odom_timestamp < pose.mlTimestamp)
        if (last_data_time_ == 0 || last_data_time_ < pose.mlTimestamp)
        {
          ...
          // latest_odom_timestamp = pose.mlTimestamp;
          uint64_t t2 = gomros::common::GetCurrentTime_us();
      ...
      void *CartoMapping::HandleImu(void *ptr)
      {
        ...
        int record_index = 0;
        while (!p->m_ImuList.empty() || p->add_data_)
        {
          if (!p->m_ImuList.empty())
          {
            // pthread_mutex_lock(&p->mutex_imu_list_raw);
            ImuSensoryMessage imu_data = p->m_ImuList.front();
            p->m_ImuList.pop_front();
            // pthread_mutex_unlock(&p->mutex_imu_list_raw);
            // 在线
            if (!p->offline_mapping && p->use_imu_based)
            {
              // 封装成函数
      ...
      void CartoMapping::ImuDataConversionAndAddFunc(ImuSensoryMessage &data)
      {
        ...
        // pthread_mutex_lock(&raw_imu_list_mtx_);
        // pthread_mutex_lock(&paint_mutex);
        if (latest_imu_timestamp < 0 || latest_imu_timestamp < data.time_stamp)
        {
          trajectory_builder_->AddSensorData("imu0", ToCartoImu(data));
          latest_imu_timestamp = data.time_stamp;
        }
        else
        {
        ...
        // pthread_mutex_unlock(&paint_mutex);
        // pthread_mutex_unlock(&mutex_imu_list_raw);
      }

      void CartoMapping::StopAndOptimize()
      {
        std::unique_lock<std::mutex> locker(paint_mutex);
        // pthread_mutex_lock(&paint_mutex);
      ...
      void CartoMapping::PaintMap()
      {
        ...
        // 创建文件夹并可以指定路径
         system("mkdir -p map");
        // std::string path = "/home/fuzhou/svn/x86_branch_refactor/plugins/mapping_and_location_carto_copy/__build/test/map";
      ...
      void CartoMapping::CreatCartoModule()
      {
        ...
        SLAM_DEBUG("###############use_imu_based:%d use_odom_based:%d\n",
                   use_imu_based, use_odom_based);
        if (use_imu_based)
        {
            ...
            [this](const int trajectory_id, const ::cartographer::common::Time time,
                   ...
                   const std::unique_ptr<
                       const ::cartographer::mapping::TrajectoryBuilderInterface::
                           InsertionResult>) {});
        SLAM_INFO("Added trajectory with ID '%d'", trajectory_id);
          ...
          RadarSensoryMessage &data)
      {
        ...
        {
          uint64_t tt =
              last_time - data.mstruRadarMessage.mstruRadarHeaderData.mlTimeStamp +
              ((start_i *
                data.mstruRadarMessage.mstruRadarHeaderData.mfTimeIncreament) *
               1000000);
          SLAM_ERROR("#####tt1 = %ld\n", tt);
          tt = data.mstruRadarMessage.mstruRadarHeaderData.mlTimeStamp +
               ((start_i *
                 data.mstruRadarMessage.mstruRadarHeaderData.mfTimeIncreament) +
                timestep * (point_num - 1)) *
                   1000000 -
      ...
      void CartoMapping::ReadRadarData()
      {
        ...
        if (rawMapFileName.is_open())
        {
          ...
          while (getline(rawMapFileName, line))
          {
            ...
            // usleep(100000);
            m_RidarList_inRead.push_back(laser_data);
          }
        }
        else
        {
      ...
      void CartoMapping::ReadOdomData()
      {
        ...
        if (rawMapFileName.is_open())
        {
          while (getline(rawMapFileName, line))
          {
            ...
            m_OdomList_inRead.push_back(pose);
